...
export async function getChatsByUserId({
  ...
  try {
    ...
    if (startingAfter) {
      ...
      } catch (startingAfterError) {
        console.error('Error with startingAfter parameter:', startingAfterError);
      ...
      } catch (endingBeforeError) {
        console.error('Error with endingBefore parameter:', endingBeforeError);
        // Fallback to default query
        filteredChats = await query();
      }
    } else {
...
export async function getMessagesByChatId({ id }: { id: string }) {
  try {
    // First try with the newer message table
    try {
      return await db
        .select()
        .from(message)
      ...
      console.error('Error retrieving messages from Message_v2:', messageError);
      
      // Fall back to deprecated message table
      return await db
        .select()
...
export async function getMessageCountByUserId({
  ...
  try {
    ...
    // First try with the newer message table (Message_v2)
    try {
      ...
      console.error('Error counting messages with Message_v2:', messageError);
      
      // Fallback to the deprecated message table if needed
      const [deprecatedStats] = await db
        .select({ count: count() })
        .from(messageDeprecated)
      ...
      return deprecatedStats?.count ?? 0;
    }
  } catch (error) {
    console.error('Failed to get message count by user ID:', error);
    // Return 0 instead of throwing an error to prevent app crashes
    return 0;
  }
}
